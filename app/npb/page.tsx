...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = `NPB Scores Today (${today}) — Live Japanese Baseball`;
  return {
    title,
    ...
    twitter: {
      card: 'summary',
      title: `NPB Scores Today (${today}) | Play-O-Graph`,
      description: league.metaDescription,
    },
  };
}
...
export default async function NPBPage() {
  ...
  return (
    <>
      ...
      <SingleLeagueScoresView slots={league.slots} initialLeagues={initialLeagues} leagueSlug="npb" />
    </>
  );
}
